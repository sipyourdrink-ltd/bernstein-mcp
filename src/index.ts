...
import { createServer, BERNSTEIN_VERSION } from "./mcp.js";
...
// The only binding is the rate limiter (wrangler.toml [[ratelimits]]): no
// KV/R2/D1/DO/service binding, nothing to pivot on. The zone's single free
...
// without the binding) means unlimited.
...
async function handleVerifyPost(request: Request): Promise<Response> {
  ...
  if (wantsJson(request)) return jsonResponse(verdictJson(v), 200);
  return html(renderVerdict(got.receipt, v));
}

async function handleVerifyGet(request: Request, url: URL): Promise<Response> {
  ...
  if (wantsJson(request)) return jsonResponse(verdictJson(v), 200);
  return html(renderVerdict(receipt, v, { expected }));
}
    ...
    try {
      ...
      if (pathname === "/verify" || pathname.startsWith("/verify/")) {
        ...
        if (request.method === "POST" && pathname === "/verify") {
          if (await overLimit(request, env)) return rateLimited();
          response = await handleVerifyPost(request);
        } else if (request.method === "GET") {
          response = await handleVerifyGet(request, url);
