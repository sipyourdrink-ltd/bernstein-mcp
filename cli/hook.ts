...
import { appendRow, logError, newMeta, readMeta, readRows, rollSegment, runId, writeMeta, type Meta } from "./store.js";
...
async function onTool(agent: Agent, ev: Extract<HookEvent, { kind: "tool" }>, now: () => number, env: NodeJS.ProcessEnv): Promise<string> {
  ...
  if (meta.head.index >= SEGMENT_ROWS) {
    ...
    if (done) rollSegment(meta, done.sealed.receiptSha256, now());
  }
  writeMeta(meta);
  return "";
}
